...
---@field on_send_pr fun(pr: GitPipelinesItem|nil)|nil
...
function M.new(params)
  ...
  local on_send_pr = params.on_send_pr
  ...
  local function define_highlights()
    ...
    vim.api.nvim_set_hl(0, 'GitPipelinesSummary', { default = true, link = 'Comment' })
    ...
    vim.api.nvim_set_hl(0, 'GitPipelinesMuted', { default = true, link = 'Comment' })
  end
  ...
  local function setup_keymaps(buf)
    ...
    vim.keymap.set('n', 'n', function()
      ...
      local meta = ui.line_meta[line]
      if on_send_pr then
        on_send_pr(meta and meta.pr)
      end
    end, map_opts('Send selected PR to NPRD Internal'))
  end
  ...
  local function build_model()
    ...
    table.insert(rows, row('Keys: <CR>/o open, n notify NPRD, r refresh, q close', nil, 'GitPipelinesMuted'))
    ...
    for _, item in ipairs(state.items) do
      ...
      table.insert(rows, row(header, { url = item.url, kind = 'pr', pr = item }, item_highlight(item)))
      ...
      if item.fetch_error then
        table.insert(rows, row('  error: ' .. shorten(item.fetch_error, 92), { kind = 'error', pr = item }, 'GitPipelinesError'))
  ...
  function self.render_float()
    ...
    local lines, line_meta = render_lines(rows)
